...
app.post("/articulo", async (req, res) => {
  try {
    ...
    if (!newArticulo?.rows?.length) return res.status(500).json({ error: "Error al insertar el artículo." });
...
app.delete("/articulo/:id", async (req, res) => {
  ...
  try {
    ...
    if (resultado.rowCount === 0) return res.status(404).json({ mensaje: "Artículo no encontrado" });
